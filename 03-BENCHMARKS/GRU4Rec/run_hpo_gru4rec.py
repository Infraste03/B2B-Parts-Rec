# #03-BENCHMARKS\GRU4Rec\run_hpo_gru4rec.py (Resumable Version)
...
# GRU4Rec model. If a results file is found, it resumes from the last completed
# run.
# =============================================================================
"""Resumable Hyperparameter Optimization (HPO) Script for GRU4Rec

This script implements a comprehensive and robust hyperparameter optimization
(HPO) workflow for the GRU4Rec (Gated Recurrent Unit for Recommendation) model
using a Random Search strategy. GRU4Rec is a classic RNN-based model for
sequential recommendation.

Key Features:
-------------
- **Random Search**: Efficiently explores a large, predefined hyperparameter space
  tailored to GRU4Rec, including RNN-specific parameters like hidden size and
  number of layers.
- **Resumable Logic**: If the script is stopped and restarted, it automatically
  detects an existing results file (`hpo_results_gru4rec.csv`), loads the progress,
...
- **Adaptive Loss Configuration**: The script dynamically adjusts the negative sampling
  strategy based on the selected loss function (BPR vs. CE) for each run.
...
results file in the current directory.
    python run_hpo_gru4rec.py

To reset the search, manually delete the `hpo_results_gru4rec.csv` file.
...
if __name__ == '__main__':
    param_space = {
        ...
        'weight_decay': np.logspace(-6, -4, 3),
        'loss_type': ['BPR', 'CE'],
    ...
    num_iterations = 50
    results_file = "hpo_results_gru4rec.csv"
    ...
    while i < num_iterations:
        ...
        config_dict = {
            ...
            'stopping_step': 10, #
            'checkpoint_dir': f'saved_hpo_runs/gru4rec/{run_name}/',
