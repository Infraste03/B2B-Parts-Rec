# 03-BENCHMARKS\RepeatNet\run_stability_repeatnet_temporal.py
#2511341
# =============================================================================
# RESUMABLE STABILITY ANALYSIS SCRIPT FOR RepeatNet
# =============================================================================
"""Resumable Stability Analysis Script for RepeatNet on a Global Temporal Split
...
This script uses a global timestamp-based data splitting strategy (70-15-15),
distinct from the leave-one-out strategy used in the main benchmarks and in the
HPO phase (run_hpo_repeatnet.py). Because the HPO was tuned on the LOO split, no
prior result can be reused here: all seeds are executed fresh under the temporal
protocol, using the best hyperparameters found via LOO-based HPO.
...
- **Best Hyperparameter Usage**: Uses the best hyperparameter combination identified
  by run_hpo_repeatnet.py (selected by NDCG@20 on the LOO split).
...
Run the script directly from the command line.
    python run_stability_repeatnet_temporal.py
...
if __name__ == '__main__':
    ...
    results_file = f"stability_results_{model_name}_temporal_split.csv"

    # Best hyperparameters found via run_hpo_repeatnet.py (selected by NDCG@20 on LOO)
    best_hyperparams = {
        'embedding_size': 256,
        'hidden_size': 256,
        'joint_train': False,
    ...
    for i, seed in enumerate(seeds_to_run):
        ...
        config_dict = {
            ...
            'checkpoint_dir': os.path.join(os.path.dirname(__file__), 'saved', 'stability_temporal_analysis', run_name),
            'log_wandb': False,
        }
