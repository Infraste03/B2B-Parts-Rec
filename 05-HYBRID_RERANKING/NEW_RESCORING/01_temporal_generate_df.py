...
def main():
    ...
    OUT_ROOT = os.path.join(BASE_DIR, "05-HYBRID_RERANKING", "NEW_RESCORING_TEMPORAL", "candidate_features")
    CHECKPOINT_PATH = os.path.join(BASE_DIR, "05-HYBRID_RERANKING", "NEW_RESCORING", "SASRec-Oct-07-2025_21-23-41.pth")
    ...
    try:
        ...
        def get_temporal_split_dataframes(config_dict_base, user_map, item_map, full_meta_df):
            """
            Replica lo split Temporal (RS 70/15/15, ordine timestamp, per-utente)
            usato per allenare il checkpoint SASRec temporale, senza passare da
            file .inter fissi. Ritorna train_df, valid_df, test_df con le stesse
            colonne prodotte da load_and_prepare_split(), pronte per il merge con
            i metadati.
            """
            temporal_config_dict = dict(config_dict_base)
            temporal_config_dict['eval_args'] = {
            ...
            temporal_config = Config(model="SASRec", dataset="b2b_data", config_dict=temporal_config_dict)
            init_seed(temporal_config['seed'], temporal_config['reproducibility'])
            full_dataset = create_dataset(temporal_config)
            train_loader, valid_loader, test_loader = data_preparation(temporal_config, full_dataset)

            def _to_df(split_loader, split_name):
                inter = split_loader.dataset.inter_feat
                uid_field = full_dataset.uid_field
                iid_field = full_dataset.iid_field
                time_field = full_dataset.time_field

                raw_df = pd.DataFrame({
                    'user_id:token': full_dataset.id2token(uid_field, inter[uid_field].numpy()),
                    'item_id:token': full_dataset.id2token(iid_field, inter[iid_field].numpy()),
                    'timestamp:float': inter[time_field].numpy(),
                })

                raw_df = raw_df.reset_index(drop=True).reset_index().rename(columns={"index": "row_id"})
                raw_df = add_internal_ids(raw_df, user_map, item_map)
                raw_df['timestamp'] = pd.to_datetime(raw_df['timestamp:float'], unit='s')
                raw_df["event_date"] = raw_df["timestamp"].dt.floor("D")
                raw_df.rename(columns={'user_id:token': 'user_id', 'item_id:token': 'item_id'}, inplace=True)
                raw_df = raw_df.sort_values(["user_id", "item_id", "event_date", "row_id"]).reset_index(drop=True)
                raw_df["occ"] = raw_df.groupby(["user_id", "item_id", "event_date"]).cumcount()

                merged = pd.merge(
                    raw_df[['row_id', 'user_id_internal', 'item_id_internal', 'timestamp', 'event_date', 'user_id', 'item_id', 'occ']],
                    full_meta_df,
                    left_on=['user_id', 'item_id', 'event_date', 'occ'],
                    right_on=['user_id', 'item_id', 'request_date', 'occ'],
                    how='left',
                ...
                logging.info(f"[{split_name}] temporal split merged columns: {list(merged.columns)}")
                return merged.sort_values(by=['user_id_internal', 'timestamp', 'row_id']).reset_index(drop=True)

            return _to_df(train_loader, "train"), _to_df(valid_loader, "valid"), _to_df(test_loader, "test")

        # =============================================================================
        # LOAD TRAIN/VALID/TEST SPLITS WITH METADATA
        # =============================================================================
        train_df, valid_df, test_df = get_temporal_split_dataframes(config_dict, user_token_to_id, item_token_to_id, meta_df)
